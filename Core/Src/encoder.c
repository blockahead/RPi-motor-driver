...
static uint16_t overflow_count[NUM_OF_ENCODERS] = { 0, 0 };
static uint16_t pulse_per_rev[NUM_OF_ENCODERS] = { 0, 0 };

void encoder_count_overflow(const ENCODER_CHANNEL channel, const int8_t dir) {
	overflow_count[channel] += dir;
}
...
void encoder_clear_count(const ENCODER_CHANNEL channel) {
	...
	case ENCODER1:
		...
		overflow_count[ENCODER1] = 0;
		break;

	case ENCODER2:
		__HAL_TIM_SET_COUNTER(&htim3, 0);
		overflow_count[ENCODER2] = 0;
		break;
...
static int32_t encoder_get_count(const ENCODER_CHANNEL channel) {
	switch (channel) {
	case ENCODER1:
		return (overflow_count[ENCODER1] << 16) | __HAL_TIM_GET_COUNTER(&htim2);

	case ENCODER2:
		return (overflow_count[ENCODER2] << 16) | __HAL_TIM_GET_COUNTER(&htim3);
